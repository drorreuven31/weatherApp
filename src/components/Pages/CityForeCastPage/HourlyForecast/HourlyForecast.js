...
const HourlyForecast = ({ current, hourly, daily, timezone_offset }) => {
  ...
  const sunStateObjects = () => {
    ...
    const sunrize = {
      ...
      hour: dateFormat(
        unixToDateTime(
          calcLocalTime(daily[relevant_sunrise_day].sunrise, timezone_offset)
        ),
        "HH:MM"
      ),
    ...
    const sunset = {
      ...
      hour: dateFormat(
        unixToDateTime(
          calcLocalTime(daily[relevant_sunset_day].sunset, timezone_offset)
        ),
        "HH:MM"
      ),
  ...
  const hourComponents = () => {
    return CreateObjectsList().map((h, index) => {
      return !h.isSunState ? (
        <HourData
          key={h.dt}
          hour={
            index !== 0
              ? unixToDateTime(calcLocalTime(h.dt, timezone_offset))
                  .getHours()
                  .toString()
        ...
        <MainInfoBox
          boxDescription={
            <>
              ...
              <h6>{keywords["hourly_forecast"][lang.id]}</h6>
            </>
          }
        >
